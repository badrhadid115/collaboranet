...
function AvoirsTable({ loading, filteredData, setFilteredData, data }) {
  const renderCnFullID = (text, record) => {
    return (
      <>
        <Link to={`/avoirs/${record.cn_full_id}`}>{text}</Link>
        <br />
        <span className="text-muted">{record.cn_comment}</span>
      </>
    );
  };
  ...
  const handleDateFilter = (confirm) => {
    if (selectedDates.length === 2) {
      ...
      const filtered = data.filter((record) => {
        const recordDate = dayjs(record.cn_date, 'DD/MM/YYYY');
  ...
  const columns = [
    {
      title: 'Avoir',
      dataIndex: 'cn_full_id',
      key: 'cn_full_id',
      render: renderCnFullID,
      width: 200
    },
    {
      title: 'Client',
    ...
    },
    {
      ...
      dataIndex: 'cn_date',
      key: 'cn_date',
      sorter: (a, b) => dayjs(a.cn_date, 'DD/MM/YYYY') - dayjs(b.cn_date, 'DD/MM/YYYY'),
  ...
  return (
    <div className="desktop-only">
      <Table
        ...
        locale={{ emptyText: <Empty description="Aucun Avoir" /> }}
      />
    </div>
  );
}
function AvoirsMobileTable({ filteredData, loading }) {
  return (
    <div className="mobile-only">
      <Spin tip="Chargement..." size="large" spinning={loading}>
        {filteredData.length === 0 && !loading && <Empty description="Aucun Avoir" />}
        <List>
          <VirtualList data={filteredData} height={420} itemHeight={20} itemKey="cn_id">
            {(item) => (
              <Link to={`/avoirs/${item.cn_full_id}`} className="text-decoration-none">
                <List.Item key={item.cn_id}>
                  <div className="d-flex justify-content-between w-100">
                    <div className="d-flex flex-column">
                      <h5 className="m-0 ms-2">{item.cn_full_id}</h5>
                      <h6 className="m-0 ms-2">{item.client_name}</h6>
                    </div>
                    <div className="d-flex flex-column align-items-end">
                      <h6 className="m-0">{item.cn_date}</h6>
...
export { AvoirsTable, AvoirsMobileTable };

AvoirsTable.propTypes = {
  loading: PropTypes.bool,
  filteredData: PropTypes.array,
  setFilteredData: PropTypes.func,
  data: PropTypes.array
};
AvoirsMobileTable.propTypes = {
  filteredData: PropTypes.array,
  loading: PropTypes.bool
};
