...
const Avoirs = () => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(apiLinks.GET.creditNotes);
  ...
  const handleSearch = (value) => {
    if (value && value.trim() !== '') {
      const fuse = new Fuse(data, { keys: ['cn_full_id', 'client_name', 'file_full_id', 'devis_formatted_id'], threshold: 0.2 });
  ...
  const renderAutocompleteOptions = (options) => {
    return options.map((option) => ({
      ...
      label: (
        <List.Item key={option.cn_id}>
          <Link className="text-decoration-none" to={`/avoirs/${option.cn_full_id}`}>
            <List.Item.Meta
              title={
                <div className="d-flex justify-content-between">
                  <span className="fw-bold fs-6">{option.cn_full_id}</span>
        ...
        <Col className="text-end my-3 d-flex align-items-center justify-content-end" md={18}>
          <Tooltip title="Télécharger la liste des bons de livraison">
            <Button
              ...
              size="large"
              onClick={() => {
                downloadExcel(data, excelOptions.creditNotes);
              }}
            />
          </Tooltip>
        </Col>
                ...
                {(item) => (
                  <List.Item key={item.cn_id}>
                    <List.Item.Meta
                      title={
                        <div className="d-flex justify-content-between">
                          <span className="fw-bold">{item.cn_full_id}</span>
