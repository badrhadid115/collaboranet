...
const AddClient = ({ open, onCancel, onSuccess }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    client_email: Yup.string().email('Adresse e-mail invalide'),
    client_number1: Yup.string().matches(/^\+?[0-9() ]+$/, 'Numéro de téléphone invalide (chiffres et + uniquement)'),
    client_number2: Yup.string().matches(/^\+?[0-9() ]+$/, 'Numéro de téléphone invalide (chiffres et + uniquement)'),
  ...
  const handleSubmit = (values, { setSubmitting, resetForm }) => {
    const response = axios.post(apiLinks.POST.clients, values);
    response
      .then((data) => {
        resetForm();
        Swal.fire({
  ...
  return (
    <Modal title="Ajouter un Client" open={open} width={1000} footer className="rounded shadow" onCancel={onCancel}>
